...
/**
 * HERO-Kunden, die im Admin bewusst gelöscht wurden. Ohne diese Sperre legt der
 * Sync sie beim nächsten Lauf wieder an – für den Anwender sieht es dann so aus,
 * als ließen sich Kunden nicht löschen.
 */
async function loadDeletedHeroCustomerIds(db) {
  const snapshot = await db.collection('heroCustomerDeletions').get()
  const ids = new Set()
  snapshot.forEach((docSnap) => {
    const data = docSnap.data() || {}
    const heroId = data.heroCustomerId != null ? String(data.heroCustomerId) : docSnap.id
    if (heroId) ids.add(heroId)
  })
  return ids
}
...
async function upsertHeroCustomer(db, customer, ctx) {
  ...
  // Bewusst gelöschte Kunden nicht wieder anlegen
  if (ctx.deletedHeroCustomerIds.has(customerId)) {
    ctx.customerStats.skippedDeleted += 1
    return null
  }
 ...
 */
async function syncHeroContactsToFirestore(db, now, deletedHeroCustomerIds) {
  ...
  const stats = { created: 0, updated: 0, skipped: 0, skippedDeleted: 0, total: 0 }
  ...
  for (const contact of contacts) {
    ...
    // Im Admin gelöschte Kunden bleiben gelöscht
    if (deletedHeroCustomerIds.has(customerId)) {
...
async function syncHeroProjectsToFirestore() {
  ...
  // Im Admin gelöschte HERO-Kunden werden von diesem Sync nicht wieder angelegt
  const deletedHeroCustomerIds = await loadDeletedHeroCustomerIds(db)

  // Alle Kunden über die contacts-Query importieren (auch ohne Projekt)
  const contactsImport = await syncHeroContactsToFirestore(db, now, deletedHeroCustomerIds)

  // Kontext für die Projekt-Kunden-Verknüpfung. Fallback-Upsert nur für
  // Projekt-Kunden, die nicht über contacts erfasst wurden.
  const customerCtx = {
    now,
    deletedHeroCustomerIds,
    existingCustomersByHeroId: await loadCustomersByHeroId(db),
    docIdByHeroCustomerId: contactsImport.docIdByHeroCustomerId,
    customerStats: { created: 0, updated: 0, skippedDeleted: 0, total: 0 }
  }
  ...
  const customerStats = {
    ...
    skippedDeleted: contactsImport.stats.skippedDeleted + customerCtx.customerStats.skippedDeleted,
