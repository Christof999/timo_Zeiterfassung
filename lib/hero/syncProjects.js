...
function buildMaterialFromProduct(product) {
  ...
  // base_price entspricht in HERO dem Einkaufs-/Grundpreis (Einkaufspreis).
  // Wird nur als Vorbelegung bei NEUEN Artikeln übernommen; manuell im Admin
  // gepflegte Einkaufspreise werden beim Sync NICHT überschrieben (siehe unten).
  const purchase = typeof product.base_price === 'number' ? product.base_price : undefined
  ...
  return {
    ...
    purchasePriceEur: typeof purchase === 'number' ? Math.round(purchase * 100) / 100 : undefined
  }
}
...
async function syncHeroMaterialsToFirestore() {
  ...
  for (const product of products) {
    ...
    // Basis-Payload OHNE Einkaufspreis: beim Update von Bestandsartikeln bleibt
    // ein manuell gepflegter purchasePriceEur erhalten (nicht vom Sync überschrieben).
    const payload = {
      name: mapped.name,
    ...
    if (existing) {
      // Einkaufspreis nur setzen, wenn am Artikel noch keiner hinterlegt ist
      // (manuelle Werte gewinnen). HERO base_price dient dann nur als Erstbefüllung.
      const existingHasPurchase =
        existing.data && typeof existing.data.purchasePriceEur === 'number'
      const updatePayload =
        !existingHasPurchase && typeof mapped.purchasePriceEur === 'number'
          ? { ...cleaned, purchasePriceEur: mapped.purchasePriceEur }
          : cleaned
      await db.collection('materialTypes').doc(existing.id).set(updatePayload, { merge: true })
      stats.updated += 1
    } else {
      const createPayload =
        typeof mapped.purchasePriceEur === 'number'
          ? { ...cleaned, purchasePriceEur: mapped.purchasePriceEur }
          : cleaned
...
async function syncProjectOfferToFirestore(projectMatchId) {
  ...
  await snap.docs[0].ref.set(
    { offerPositions: built.positions, offerMeta: built.offerMeta },
    { merge: true }
  )
    ...
    try {
      ...
      if (built.found) {
        ...
        await db
          .collection('projects')
          .doc(entry.id)
          .set({ offerPositions: built.positions, offerMeta: built.offerMeta }, { merge: true })
        stats.withOffer += 1
      }
    } catch (e) {
      stats.errors += 1
